...
- **Sharing** — invite by email (including people with no account yet) with an optional **Notify by email** checkbox that sends a branded invitation, share links scoped to Editor, Commenter or Viewer, instant revocation that disconnects live sessions.
...
- **Comments** — select text and comment; threads with replies, resolve/reopen, live for everyone in the document. A comment stays attached to its words while others type around it, and says so when those words are gone.
- **Mentions** — type @ in a comment to mention someone who can open the document. They get an in-app notification record and an email (with a link that opens the thread), unless they turned that off in the account menu or are already looking at the document.
- **Notifications** — a bell in the dashboard and the editor with an unread badge. Mentions appear instantly if you have a document open (a per-person socket room), and within a minute or on returning to the tab otherwise. Choosing one opens the document on that thread.
- **Roles** — Owner / Editor / Commenter / Viewer (a Commenter reads and comments but can't change the text), enforced server-side on every REST call and every socket message.
...
**Comments are pinned to the text with Yjs relative positions.** A character offset stops pointing at the right words the moment anyone types before it. A Yjs relative position points at the letters themselves, so it follows the text through concurrent edits; the editor converts each thread's pair of positions back into a highlight whenever the document changes. If the words are deleted (or the document is restored to an older version) the thread isn't lost: it keeps the quoted text and is marked as detached. The server treats an anchor as opaque, untrusted JSON (shape and size checked, never interpreted), and the editor validates it before Yjs sees it, because resolving a position whose type name is unknown makes Yjs create a root type of that name. Comment bodies are plain text, rendered as text. Every comment route re-checks the caller's role, requires the thread to belong to the document in the URL, and caps threads per document (500) and replies per thread (200). Changes are saved over REST and pushed to the document's socket room (and across server instances over Redis), and the list is reloaded after each reconnect so a missed event can't leave a stale panel.

**Mentions are ids, decided by the server.** The comment text stays plain (`@Grace Hopper`) and the people it names travel beside it as ids. The server keeps only ids that belong to people who can open the document (never yourself, never a stranger, duplicates collapsed), because a mention triggers an email and a notification. The autocomplete list comes from an endpoint that returns names only, so a commenter can't harvest addresses. Mention emails are fenced the same way invitations are (per person's own switch and presence, a cooldown per thread and recipient, hourly caps per author and per recipient, and the shared daily budget, which stops short of the sign-in reserve), never delay the comment, and never fail it. Editing a comment only notifies people who weren't already mentioned. The link in the email is a plain link into the app (`?thread=<id>`) with no token: signed out, it goes through sign-in and lands on the thread.

**The bell only shows what you can still open.** A notification is a row per mention, and every read joins against the documents you own or are shared on, so losing access to a document also hides its notifications (they come back if access does; nothing is deleted). Reads are scoped to the signed-in user inside the SQL, and marking read only ever touches your own rows. The live push goes to a personal socket room and, like comment changes, across server instances over Redis; if it's missed, the bell's own refresh catches up.
...
| Comments | Role checked on every route (read: any access; write: owner/editor/commenter; unknown documents answer 404); thread must belong to the document in the URL; anchors validated on both sides; bodies rendered as text; per-user write rate limit and per-document ceilings |
| Mentions | Only people who can open the document can be mentioned (server-filtered ids); names-only people list; per-recipient switch; email fenced by cooldown, per-author and per-recipient caps and the shared daily budget; comment text escaped in the email |
| Notifications | Scoped to the caller in SQL; hidden once access to the document is lost; per-user read rate limit; the push carries no content, only "refresh" |
